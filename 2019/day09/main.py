class int_computer:
    ...
    def get_params(self, p1mode, p2mode, p3mode, opcode):
        ...
        if p1mode == 2:
            ...
            p1 = (
                self.code[self.pointer + 1]
                if p1mode
                else self.code[self.code[self.pointer + 1]]
            )
        if opcode in [1, 2, 5, 6, 7, 8]:
            if p2mode == 2:
                ...
                p2 = (
                    self.code[self.pointer + 2]
                    if p2mode
                    else self.code[self.code[self.pointer + 2]]
                )
...
if __name__ == "__main__":
    ...
    content = read_input("input.txt") + [
        0 for u in range(10000)
    ]  # Make computer memory larger
    # Solve p1
